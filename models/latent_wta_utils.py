...
class LatentWTA(nn.Module):
    ...
    def vq_decode(self, indices, z_q_x):
        z_q_x_bar_flatten = torch.index_select(self.vqvae.codebook.embedding.weight, dim=0, index=indices)
        z_q_x_bar = z_q_x_bar_flatten.view_as(z_q_x)
        return self.vqvae.decoder(z_q_x_bar)

    def forward(self, x, spatial=True, lifetime=True):
        z_q_x, indices_gt = self.vq_encode(x)
        z = self.encode(z_q_x)
        if spatial:
        ...
        logits_pred = self.decode(z)
        logits_pred = logits_pred.permute(0, 2, 3, 1).contiguous()
        logits_pred = logits_pred.view(-1, logits_pred.shape[-1])
        indices_pred = torch.argmax(logits_pred, dim=1)
        out = self.vq_decode(indices_pred, z_q_x)
        if self.training:
            return out, indices_gt, logits_pred
        else:
            return out
...
def train_one_epoch(model, optimizer, train_data):
    ...
    for batch_idx, (data, _) in enumerate(train_data):
        data = data.to(device)
        optimizer.zero_grad()
        recon_batch, indices_gt, logits_pred = model(data)
        loss = nn.CrossEntropyLoss()(logits_pred, indices_gt)
        loss.backward()
