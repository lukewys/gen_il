...
class SparseAttnAutoencoder(nn.Module):
    def __init__(self, latent_dim=128, ch=1, image_size=32, sample_num=100,
        ...
        self.encoder = EncoderBurgess((ch, image_size, image_size), hid_channels=hid_channels,
                                      latent_dim=self.latent_dim * num_topk)
        ...
        self.sparse_attention = SparseAttnBottleneck(voc_size, self.latent_dim, num_topk=6)
    ...
    def bottleneck(self, latent):
        emb = latent.reshape(-1, self.latent_dim)
        emb = self.embedder(emb)
        emb_after_attn = self.sparse_attention(emb)
        emb_proj = self.projector(emb_after_attn)
        emb_proj = emb_proj.reshape(-1, self.num_topk, self.latent_dim)
        emb_sum = emb_proj.sum(dim=1)
        return emb_sum
