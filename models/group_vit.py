...
class AssignAttention(nn.Module):
    ...
    def forward(self, query, key=None, *, value=None, return_attn=False):
        # query=projected_group_tokens, key=x, value=x
        # query: [B, N, C]
        B, N, C = query.shape
        if key is None:
            key = query
...
class GroupViT(nn.Module):
    ...
    def __init__(self,
        ...
        assert len(embed_factors) == len(depths) == len(num_group_tokens) == len(gumbel_taus) + 1
        ...
        for i_layer in range(self.num_layers):
            ...
            if i_layer < self.num_layers - 1:
                ...
                downsample = GroupingBlock(
                    ...
                    gumbel_tau=gumbel_taus[i_layer], )
...
# https://github.com/NVlabs/GroupViT/blob/main/segmentation/evaluation/group_vit_seg.py#L26
...
# https://github.com/NVlabs/GroupViT/blob/main/segmentation/evaluation/group_vit_seg.py#L144
def get_attn_maps(results, num_patch_h, num_patch_w, return_onehot=False, rescale=False):
    """
    Args:
        img: [B, C, H, W]
    Returns:
    ...
    with torch.no_grad():
        ...
        for idx, attn_dict in enumerate(results['attn_dicts']):
            ...
            attn_maps.append(resize_attn_map(prev_attn_masks, num_patch_h, num_patch_w))
    ...
    for i in range(len(attn_maps)):
        ...
        #         attn_map, size=img.shape[2:], mode='bilinear', align_corners=align_corners)
        ...
        # number of different group token type used in average
        token_usage = attn_map.sum((1, 2))
        token_usage = (token_usage > 0).float().sum(-1)
        token_usage = token_usage.mean()
        print(f'attn_maps[{i}] token_usage: {token_usage:.2f} / {attn_map.shape[-1]}')
...
if __name__ == '__main__':
    ...
    out = model(x, return_feat=True, return_attn=True, as_dict=True)
    attn_maps = get_attn_maps(out, return_onehot=True, rescale=False)
    # out['feat']: [B, N, C]
    # attn_maps[-1]: [B, H, W, G]
